...
def main(output_dir):
    ...
    mnfx = [np.zeros_like(rho) for _ in range(4)]  # overlap, vi, nmi, Bs
    stdfx = [np.zeros_like(rho) for _ in range(4)]
    bstfx = [np.zeros_like(rho) for _ in range(4)]
    ...
    for i in tqdm(range(n)):
        for j in range(m):
            ...
            mnfx[0][i, j], mnfx[1][i, j], mnfx[2][i, j], mnfx[3][i, j] = mn_res
            stdfx[0][i, j], stdfx[1][i, j], stdfx[2][i, j], stdfx[3][i, j] = std_res
            bstfx[0][i, j], bstfx[1][i, j], bstfx[2][i, j], bstfx[3][i, j] = best_res
    ...
    if not args.nosave:
        ...
        filename = (
            f"{sign}_{gamma_name}rho-lamb_{model}_{nb_repeats}_{nb_net_repeats}.npz"
        )
